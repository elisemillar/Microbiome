#
#title: "Hydropsychidae Microbiome"
#author: "Elise Millar"
#date: "Aug 27th, 2020"
...
#Subset to only ~Hydropsychidae~ samples
hydat <- subset_samples(data, Sample_Label=="Hydropsychidae")
hydat <- prune_taxa(taxa_sums(hydat) >0, hydat)
hydat #5282 ASVs, 59 samples
...
effgen = subset_taxa(hydat, (Genus %in% effluent))
effgen <- prune_samples(sample_sums(effgen) >0, effgen)
efflist = psmelt(effgen)
efflist[1:30, c(1:4,33)]

#By Phylum (Ex. Cyanobacteria)
cyano = subset_taxa(hydat, Phylum=="Cyanobacteria")
cyano

#By Site (Ex. REF1)
REF1 <- subset_samples(hydat, Site_ID=="REF1")
...
get_taxa_unique(hydat, "Phylum") #25
get_taxa_unique(hydat, "Class") #50
get_taxa_unique(hydat, "Order") #130
get_taxa_unique(hydat, "Family") #173
get_taxa_unique(hydat, "Genus") #298

#################################################### Abundance Numbers ############################################################################

##Total number of sequence reads for dataset (including unassigned)
sum(sample_sums(hydat)) #1374330
#Read sequence range for samples
sort(sample_sums(hydat)) #1317-37485

#Number of sequence reads per rank (Ex. Phylum)
top.taxa = sort(tapply(taxa_sums(hydat), tax_table(hydat)[, "Phylum"], sum), TRUE)

top.taxa #highest amount of reads to lowest
top.taxa[1:10] #top 10 taxa by read count
sum(top.taxa) #total reads assigned to taxa = 1374158
...
rare.depth <- min(sample_sums(hydat))
rare.depth #1317
dat_r <- rarefy_even_depth(hydat, sample.size = rare.depth, rngseed=1414) #rngseed = random value
dat_r
...
alpha_div$Sample_ID <- c(415:419, 440:445, 451:453, 460:465, 485:489, 495:502, 508:513, 523:527, 538:548, 559:562) #exact range for Hydropsychidae samples in metadata
...
p = ggplot(data=alpha_div_merge, aes(x=Site_ID, y=Shannon)) + 
...
#################################################### Alpha Diveristy Values ##################################################

#Winner: middle
ggplot_build(p)$data

#Need Shannon and Site_ID from:
alpha_div_merge

alphavalues = alpha_div_merge[ , c('Shannon','Site_ID')] #select columns from metadata
alphavalues

tapply(alphavalues$Shannon, alphavalues$Site_ID, mean)
...
trans.dat <- transform_sample_counts(hydat, function(otu) otu/sum(otu))
...
bray.plot = plot_ordination(trans.dat, bray.ord, color="Site_ID", title="Bray-Curtis Hydropsychidae", axes = c(1,2)) + 
...
pairwise.adonis(bray.test, sample_data(hydat)$Location_vs_WWTP, perm = 99999) #by Location
pairwise.adonis(bray.test, sample_data(hydat)$Site_ID, perm = 99999) #by Site
...
phylaColours1 = c(Acidobacteria = "#49e3d9", Actinobacteria = "#e45ad1", Bacteroidetes = "#a8e14b", 
                 Chloroflexi = "#9e73f2", Cyanobacteria = "#decf3f", Deferribacteres = "#5686ed", 
                 Elusimicrobia = "#d780d1", Epsilonbacteraeota = "#6fe785", Firmicutes = "#e66aa1", 
                 ...
                 Verrucomicrobia = "#cde086")

phylaColours2 = c(Bacteroidetes = "#e5e736", Cyanobacteria = "#43e07b", Deferribacteres = "#5686ed", 
                  Firmicutes = "#eab42c", Planctomycetes = "#ee603a", Proteobacteria = "#8fd9e5", 
                  RsaHf231 = "#a19d2e", Synergistetes = "#e45ad1", `< 2% abund.` = "#A9A9A9")

########################################################
#Phylum - remove taxa in <2% abundance (normalize)
dat.aglo = tax_glom(hydat, taxrank = "Phylum")
...
dat.agr$Phylum <- factor(dat.agr$Phylum, levels = c("Bacteroidetes", "Cyanobacteria", "Deferribacteres", "Firmicutes", 
                                                    "Planctomycetes", "Proteobacteria", "RsaHf231", "Synergistetes", 
                                                    "< 2% abund."))
dat.agr$Site_ID= factor(dat.agr$Site_ID, 
                        levels=c("REF1", "REF2", "REF3", "DSW1", "DSW2", "DSW3", "DSK1", "DSK2", "DSK3", "DSK4"))
dat.agr$Location_vs_WWTP= factor(dat.agr$Location_vs_WWTP, 
...
cyano = hydat %>% subset_taxa(Phylum == "Cyanobacteria")
cyano
...
cyanogen = c('Tychonema_CCAP_1459-11B', 'Cyanobium_PCC-6307', 'Calothrix_UAM_374',
             'Planktothrix_NIVA-CYA_15', 'Pleurocapsa_PCC-7319', 'Calothrix_KVSF5')
cygen = subset_taxa(hydat, (Genus %in% cyanogen))
...
endo = subset_taxa(hydat, (Genus %in% endobac))
endo
...
REF_DSW = subset_samples(hydat, Location_vs_WWTP=="Downstream_Waterloo" | Location_vs_WWTP=="Upstream")
...
REF_DSK = subset_samples(hydat, Location_vs_WWTP=="Downstream_Kitchener" | Location_vs_WWTP=="Upstream")
...
DSW_DSK = subset_samples(hydat, Location_vs_WWTP=="Downstream_Waterloo" | Location_vs_WWTP=="Downstream_Kitchener")
