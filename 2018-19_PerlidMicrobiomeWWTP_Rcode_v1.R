...
#Subset to only ~Perlidae~ samples
perdat <- subset_samples(data, Sample_Label=="Perlidae")
perdat <- prune_taxa(taxa_sums(perdat) >0, perdat)
perdat #8847 ASVs, 55 samples
...
effgen = subset_taxa(perdat, (Genus %in% effluent))
effgen <- prune_samples(sample_sums(effgen) >0, effgen)
efflist = psmelt(effgen)
efflist[1:30, c(1:4,33)]

#By Phylum (Ex. Cyanobacteria)
cyano = subset_taxa(perdat, Phylum=="Cyanobacteria")
cyano

#By Site (Ex. REF1)
REF1 <- subset_samples(perdat, Site_ID=="REF1")
...
get_taxa_unique(perdat, "Phylum") #35
get_taxa_unique(perdat, "Class") #66
get_taxa_unique(perdat, "Order") #165
get_taxa_unique(perdat, "Family") #239
get_taxa_unique(perdat, "Genus") #476

#################################################### Abundance Numbers ############################################################################

##Total number of sequence reads for dataset (including unassigned)
sum(sample_sums(perdat)) #1801248
#Read sequence range for samples
sort(sample_sums(perdat)) #2134-72461

#Number of sequence reads per rank (Ex. Phylum)
top.taxa = sort(tapply(taxa_sums(perdat), tax_table(perdat)[, "Phylum"], sum), TRUE)

top.taxa #highest amount of reads to lowest
top.taxa[1:10] #top 10 taxa by read count
sum(top.taxa) #total reads assigned to taxa = 1800757
...
ntaxa(perdat)
tax_table(perdat)
top_taxa = table(tax_table(perdat)[, "Phylum"], exclude = NULL) #Ex. Phylum
top_taxa
sort(top_taxa, decreasing = TRUE)
head(sort(top_taxa, decreasing = TRUE))

#ASV counts per Species
asv_df <- t(otu_table(perdat))
colSums(asv_df != 0)
###############
...
rare.depth <- min(sample_sums(perdat))
rare.depth #2134
dat_r <- rarefy_even_depth(perdat, sample.size = rare.depth, rngseed=1414) #rngseed = random value
dat_r
...
alpha_div$Sample_ID <- c(420:429, 435:439, 454, 471:484, 503:507, 514:518, 533:537, 549:553, 566:570) #exact range for Perlidae samples in metadata
...
trans.dat <- transform_sample_counts(perdat, function(otu) otu/sum(otu))
...
bray.plot = plot_ordination(trans.dat, bray.ord, color="Site_ID", title="Bray-Curtis Perlidae", axes = c(1,2)) + 
...
pairwise.adonis(bray.test, sample_data(perdat)$Location_vs_WWTP, perm = 99999) #by Location
pairwise.adonis(bray.test, sample_data(perdat)$Site_ID, perm = 99999) #by Site

################################################## Relative Abundance Plots ########################################################################

#colours
library(scales)
show_col(hue_pal()(6)) #ggplot colours, can adjust to amount of taxa. Didn't use but handy to have.
...
dat.aglo = tax_glom(perdat, taxrank = "Phylum")
...
dat.agr$Phylum <- factor(dat.agr$Phylum, levels = c("Acidobacteria", "Bacteroidetes", "Cyanobacteria", "Firmicutes", 
...
cyano = perdat %>% subset_taxa(Phylum == "Cyanobacteria")
cyano
...
# Relative abundance matrices (Ex. Snowella_0TU37S04)
with(dat.agr[dat.agr$Genus == "Snowella_0TU37S04", ], table(Abundance, Site_ID))
...
cyanogen = c('Tychonema_CCAP_1459-11B', 'Cyanobium_PCC-6307', 'Snowella_0TU37S04',
             'Planktothrix_NIVA-CYA_15', 'Pleurocapsa_PCC-7319', 'Calothrix_KVSF5')
cygen = subset_taxa(perdat, (Genus %in% cyanogen))
...
endo = subset_taxa(perdat, (Genus %in% endobac))
endo
...
REF_DSW = subset_samples(perdat, Location_vs_WWTP=="Downstream_Waterloo" | Location_vs_WWTP=="Upstream")
...
REF_DSK = subset_samples(perdat, Location_vs_WWTP=="Downstream_Kitchener" | Location_vs_WWTP=="Upstream")
...
DSW_DSK = subset_samples(perdat, Location_vs_WWTP=="Downstream_Waterloo" | Location_vs_WWTP=="Downstream_Kitchener")
