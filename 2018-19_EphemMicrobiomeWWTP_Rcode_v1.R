#
#title: "Ephemerellidae Microbiome"
#author: "Elise Millar"
#date: "Aug 27th, 2020"
...
#Subset to only ~Ephemerellidae~ samples
ephdat <- subset_samples(data, Sample_Label=="Ephemerellidae")
ephdat <- prune_taxa(taxa_sums(ephdat) >0, ephdat)
ephdat #3672 ASVs, 10 samples
...
effgen = subset_taxa(ephdat, (Genus %in% effluent))
effgen <- prune_samples(sample_sums(effgen) >0, effgen)
efflist = psmelt(effgen)
efflist[1:30, c(1:4,33)]

#By Phylum (Ex. Cyanobacteria)
cyano = subset_taxa(ephdat, Phylum=="Cyanobacteria")
cyano

#By Site (Ex. REF1)
REF1 <- subset_samples(ephdat, Site_ID=="REF1")
...
get_taxa_unique(ephdat, "Phylum") #36
get_taxa_unique(ephdat, "Class") #64
get_taxa_unique(ephdat, "Order") #151
get_taxa_unique(ephdat, "Family") #225
get_taxa_unique(ephdat, "Genus") #382

#################################################### Abundance Numbers ############################################################################

##Total number of sequence reads for dataset (including unassigned)
sum(sample_sums(ephdat)) #361788
#Read sequence range for samples
sort(sample_sums(ephdat)) #21997-46243

#Number of sequence reads per rank (Ex. Phylum)
top.taxa = sort(tapply(taxa_sums(ephdat), tax_table(ephdat)[, "Phylum"], sum), TRUE)

top.taxa #highest amount of reads to lowest
top.taxa[1:10] #top 10 taxa by read count
sum(top.taxa) #total reads assigned to taxa = 361762
...
ntaxa(ephdat)
tax_table(ephdat)
top_taxa = table(tax_table(ephdat)[, "Phylum"], exclude = NULL) #Ex. Phylum
top_taxa
sort(top_taxa, decreasing = TRUE)
head(sort(top_taxa, decreasing = TRUE))

#ASV counts per Species
asv_df <- t(otu_table(ephdat))
colSums(asv_df != 0)
###############
...
rare.depth <- min(sample_sums(ephdat))
rare.depth #21997
dat_r <- rarefy_even_depth(ephdat, sample.size = rare.depth, rngseed=1414) #rngseed = random value
dat_r

#Plot by Site_ID
alpha_plot = plot_richness(dat_r, x="Site_ID", measures=c("Shannon", "Simpson"))
newlabelorder_Site_ID = c("REF1", "REF2")
...
alpha_div$Sample_ID <- c(430:434, 446:450) #exact range for Ephemerellidae samples in metadata
...
#Site colours
SiteColours = c(REF1 = "#F8766D", REF2 = "#DE8C00")
...
newlabelorder_Location = c("Upstream")
newlabelname_Location = c("Upstream")
newlabelorder_Site = c("REF1", "REF2")

SiteColours = c(REF1 = "#F8766D", REF2 = "#DE8C00")

# Transform data to proportions for Bray-Curtis distances
trans.dat <- transform_sample_counts(ephdat, function(otu) otu/sum(otu))
...
bray.plot = plot_ordination(trans.dat, bray.ord, color="Site_ID", title="Bray-Curtis Ephemerellidae", axes = c(1,2)) + 
...
pairwise.adonis(bray.test, sample_data(ephdat)$Site_ID, perm = 99999) #by Site
...
dat.aglo = tax_glom(ephdat, taxrank = "Phylum")
...
dat.agr$Phylum <- factor(dat.agr$Phylum, levels = c("Bacteroidetes", "Cyanobacteria", "Proteobacteria", "< 2% abund."))
dat.agr$Site_ID= factor(dat.agr$Site_ID, 
                        levels=c("REF1", "REF2"))
dat.agr$Location_vs_WWTP= factor(dat.agr$Location_vs_WWTP, 
                                 levels=c("Upstream"), 
...
cyano = ephdat %>% subset_taxa(Phylum == "Cyanobacteria")
cyano
...
cyanogen = c('Tychonema_CCAP_1459-11B', 'Cyanobium_PCC-6307', 'Chamaesiphon_PCC-7430',
             'Calothrix_PCC-6303', 'Pleurocapsa_PCC-7319', 'Calothrix_KVSF5')
cygen = subset_taxa(ephdat, (Genus %in% cyanogen))
...
endo = subset_taxa(ephdat, (Genus %in% endobac))
endo
